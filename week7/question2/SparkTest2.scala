...
case class SparkTestCpOptions(source: String,
                              target: String,
                              maxConcurrence: Int,
...
object SparkTest2 {
  ...
  def run(opts: SparkTestCpOptions, spark: SparkContext,
        ...
        try {
          ...
          case e: Exception => {
            if (opts.ignoreFailures) {
              log.warn("file: {} failed.", copyInfo.sourceFile, e)
            } else {
              throw e
            }
          }
  ...
  def getCopyFileList(distCpOptions: SparkTestCpOptions): ArrayBuffer[CopyInfo] = {
  ...
  def parseOpts(args: Array[String]): SparkTestCpOptions = {
    ...
    SparkTestCpOptions(source, target, maxConcurrence, ignoreFailures, appName, master)
  }
